...
def day_overview(book_date: date) -> dict:
    ...
    totals = compute_totals(book_date) if not day or not day.locked else {
        "bri": day.total_in_bri, "bca": day.total_in_bca,
        "merchant_bca": day.total_in_merchant_bca, "mandiri": day.total_in_mandiri,
        "bank": day.total_in_bank, "otomax": day.total_out_otomax,
...
def open_discrepancies_before(book_date: date):
    return Discrepancy.objects.filter(
        status=DiscrepancyStatus.OPEN, origin_book_date__lt=book_date
...
def alarm_discrepancies(today: date | None = None):
    ...
    return Discrepancy.objects.filter(
        status=DiscrepancyStatus.OPEN, origin_book_date__lte=cutoff
    ).order_by("origin_book_date")


def cumulative_open_total():
    return Discrepancy.objects.filter(status=DiscrepancyStatus.OPEN).aggregate(
        s=Sum("amount")
    )["s"] or 0
