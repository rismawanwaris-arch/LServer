...
def carry_forward(book_date: date, user=None) -> int:
    ...
    resolved = 0
    stale = Discrepancy.objects.filter(
        status="OPEN", origin_book_date__lt=book_date
    ...
    for disc in stale:
        ...
        if match:
            resolve_discrepancy(
                disc, match=match, resolution_type="LATE_MATCH", user=user, on_date=book_date
            )
            resolved += 1
    return resolved
...
def _find_otomax_for(bank: BankMutation, book_date: date) -> Match | None:
    o = (
        OtomaxEntry.objects.filter(
            ...
            channel_hint=bank.channel, amount=bank.amount, match_status=MatchStatus.UNMATCHED,
        )
    ...
    o = o or OtomaxEntry.objects.filter(
        book_date=book_date, category=OtomaxCategory.TOPUP_TARTUN, channel_hint=bank.channel,
        amount=bank.amount, ref_normalized=bank.ref_normalized, match_status=MatchStatus.UNMATCHED,
    ).first()
    if not o:
        return None
...
def _find_bank_for(otomax: OtomaxEntry, book_date: date) -> Match | None:
    ...
    b = BankMutation.objects.filter(
        book_date=book_date, channel=channel, amount=otomax.amount,
        match_status=MatchStatus.UNMATCHED,
    ).filter(ref_normalized=otomax.ref_normalized).first()
    if not b and otomax.ref_core:
        b = BankMutation.objects.filter(
            book_date=book_date, channel=channel, amount=otomax.amount,
            ref_core=otomax.ref_core, match_status=MatchStatus.UNMATCHED,
        ).first()
    if not b:
        return None
...
def _persist(book_date, channel, bank, otomax) -> Match:
    match = Match.objects.create(
        ...
        match_type=MatchType.MANUAL, amount_bank=bank.amount, amount_otomax=otomax.amount,
        note="carry-forward",
    )
